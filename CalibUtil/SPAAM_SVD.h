...
class SPAAM_SVD
{
	...
	private:
		...
		public: ublas::matrix< double, boost::numeric::ublas::column_major> Proj3x4;
		...
		void clearMatrix()
		{
			...
			modMatrixScreen.resize(3, 3);
			modMatrixWorld.clear();
			modMatrixWorld.resize(4, 4);
			memset(projMat3x4, 0, 16*sizeof(double));
		}
		...
		calculations. The function should be called on each set of points from each domain.
		It will normalize a vector of points whoe first and last values are pointed to by the
		iterators iBegin and iEnd. The functions taks 4 arguments as follows:

		Template Parameters: 
		T: represents the data type desribing the value of the points being normalized
		(i.e. double, float, int, etc.)
		N: represents the number of dimensions (values) contained within each point (2D, 3D, etc.)
		ForwardIterator: represents the type of iterator describing iBegin and iEnd (const_vector_iterator)

		const ForwardIterator iBegin: This is an iterator which points to the beginning of the
		the list/vector of points to be normalized

		const ForwardIterator iEnd: This is an iterator which points to the end of the
		list/vector of points to be normalized

		ublas::c_vector< T, N >& shift: This is a single point vector which represents the
		value amount that the normalization had to shift. It is passed by reference

		ublas::c_vector< T, N >& scale: This is a single point vector which represents the
		value amount that the normalization had to resize (scale). It is passed by reference
		...
		*****************************************************************************/
		...
		the resulting matrix back into real space. The function should be called on each set
		of normalized points from each domain. The functions taks 3 arguments as follows:

		Template Parameters: 
		T: represents the data type desribing the value of the normalized points (i.e. double,
		float, int, etc.)
		N: represents the number of dimensions (values) contained within each point (2D, 3D, etc.)

		const ublas::c_vector< T, N >& shift
		const ublas::c_vector< T, N >& scale
		const bool modInverse

		The return value is the normalization matrix for the set of points
		ublas::matrix< T, boost::numeric::ublas::column_major >
		*****************************************************************************/
		...
		void constructProjectionMatrix4x4_(double*& final, double* m, float ne, float fr, int right, int left, int top, int bottom)
		{
			...
			double norm = std::sqrt(proj4x4[8] * proj4x4[8] + proj4x4[9] * proj4x4[9] + proj4x4[10] * proj4x4[10]);
		...
		ublas::matrix< double, boost::numeric::ublas::column_major> projectionDLTImpl( )
		{
			...
			double fViewDirLen = std::sqrt( Proj3x4( 2, 0 ) * Proj3x4( 2, 0 ) + Proj3x4( 2, 1 ) * Proj3x4( 2, 1 ) + Proj3x4( 2, 2 ) * Proj3x4( 2, 2 ) );
