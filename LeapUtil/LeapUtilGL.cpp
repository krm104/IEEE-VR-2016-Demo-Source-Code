...
void drawCylinder( eStyle style, eAxis axis )
{
  ...
  gluCylinder( s_quadric, .5f, .5f, 1.0f, 32, 32 );
...
void drawArrow( eAxis axis )
{
  ...
  void OcludedrawSkeletonHand(const Leap::Hand& hand, const GLVector4fv& vBoneColor, const GLVector4fv& vJointColor) {
	  static const float kfJointRadiusScale = 0.75f;
	  static const float kfBoneRadiusScale = 2.f;
	  static const float kfPalmRadiusScale = 3.15f;
	  ...
	  for (int i = 0, ei = fingers.count(); i < ei; i++) {
		  ...
		  // draw segment of metacarpal box
		  glColor4fv(vBoneColor);
		  drawCylinder(kStyle_Solid, vCurBoxBase, vLastBoxBase, 2.5*kfBoneRadiusScale*fRadius);
		  glColor4fv(vJointColor);
		  vLastBoxBase = vCurBoxBase;
	  }
